...
npm run provider -- gmail            # regenerate this repo
npm run provider -- gmail --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/gmail-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/gmail-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/gmail-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-gmail` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/gmail/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/gmail` |
| npm | `@particle-academy/gmail-ui`, `@particle-academy/gmail-js` |
| Packagist | `particle-academy/gmail-php` |
| PyPI | `fancy-gmail` |
...
**Default to plain HTTP.** A Gmail SDK is third-party code and is subject to
